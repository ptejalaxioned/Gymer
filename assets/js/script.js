...
form.addEventListener("submit", (e) => {
    ...
    //For subject
    if (subject_div.children.length == 2) {
      let span_subject = document.createElement("span");
      span_subject.classList.toggle("div-span");
      subject_div.appendChild(span_subject);
      addSpan(span_subject, subject , e, regx_Sname, span_content_sbject);
    } else {
      let span_subject = subject_div.children[2];
 ...
 if (textarea_div.children.length == 2) {
    ...
    let span_textarea =  textarea_div.children[2];
    addSpanForTextarea(
      span_textarea,
      textarea,
      e
...
function addSpanForTextarea(span, input) {
    if (input.value == "") {
      span.innerHTML = "This field is required";
      input.classList.add("input-border");
      e.preventDefault();
